...
| --- | --- | --- | --- |
| BL-01 | 乗算コード生成で両オペランドが同じ仮想レジスタを使い、`bench.pul`が400を出力 | `bench.pul`と`bench.bin`がどちらも**9900**を出力。`MUL $r15, $r15, $r14`となり別レジスタを使用。 | **修正済み** |
...
`bench.pul`の期待値は `2 × (0 + ... + 99) = 9900` である。新ビルドではソース実行と生成済みバイトコード実行の両方で9900を返した。
...
`filter.pul`の`$rtt > 300us`は、正しく300,000 nsとして生成された。
