# PulseLang & px64 完全自律AI仕様書 兼 システムアーキテクチャ定義書 (日本語版)
...
固定長静的配列は、ヒープ割り当てを行わず、カーネル/VMの静的配列スロット領域（スクリプト全体で最大8配列、合計最大256要素）を利用して $O(1)$ の決定論的メモリアクセスを提供します。

> **メモリ・レジスタモデルと制約**:
> - **GPR制限の非消費**: 配列要素は GPR 変数スロット（`$rax`〜`$r15` の 13 変数制限）を消費せず、静的メモリスロット (`array_slots[256]`) に配置されます。これにより、13 変数制限に阻まれることなく多要素データや行列を表現できます。
> - **ハードウェア境界検査 (Bounds Check)**: 読み出し (`ARR_LOAD`) および書き込み (`ARR_STORE`) 時に $0 \le \text{index} < N$ が常時検証され、範囲外アクセス時は即座に `ERR_PX64_ARRAY_OUT_OF_BOUNDS` が発生します。
> - **3つの宣言・初期化スタイル**:
>   1. **未初期化宣言 (ゼロクリア)**: `let $a: [i64; 9];` または `let mut $a: [i64; 9];`
>   2. **型注釈付き初期化**: `let $a: [i64; 9] = [1, 2, 3, 4, 5, 6, 7, 8, 9];`
>   3. **直接配列リテラル**: `let $a = [1, 2, 3, 4, 5, 6, 7, 8, 9];`

```pulse
// 1. 配列宣言と初期化のバリエーション
let $zeros: [i64; 4];                            // 4要素 (0 でゼロ初期化)
let $typed: [i64; 3] = [10, 20, 30];            // 型注釈付き初期化
let $matrix = [                                 // 要素数から自動推論 (9要素)
    1, 2, 3,
    4, 5, 6,
    7, 8, 9
];
...
let mut $out: [i64; 9];
$out[0] := 100;
let $val = $matrix[4];                          // 5 をロード

// 3. 実践例: 3×3 行列乗算 (Matrix Multiplication)
...
> **アーキテクチャ上の主要保証**:
> - **ゼロ動的ヒープ確保 (Zero Heap)**: 中間配列やクロージャオブジェクトを一切生成しません。
> - **コンパイル時ループ融合 (Loop Fusion)**: コンビネータチェーン（例: `@zip_with(...) |> @sum()`）は、レジスタアキュムレータを用いた **単一の最適化された `px64` ループ** へ完全にインライン展開されます。
> - **静的 WCET & 次元証明**: 入力次元と関数の計算量から、コンパイル時に最悪実行時間（WCET）を自動証明し、次元不一致を実行前に 100% 検出します。

#### 1. ストライドビュー命令 (Stride View Intrinsics)
| 命令 | 引数 | 説明 |
|---|---|---|
| `@row($arr, $i, $cols)` | 配列, 行インデックス, 列数 | 開始位置 `$i * $cols`、ストライド 1、長さ `$cols` のゼロコピー行ビューを生成 |
| `@col($arr, $j, $cols)` | 配列, 列インデックス, 列数 | 開始位置 `$j`、ストライド `$cols` のゼロコピー列ビューを生成 |
| `@slice($arr, $start, $len)` | 配列, 開始インデックス, 長さ | 開始位置 `$start` から `$len` 要素の連続部分配列ビューを生成 |

#### 2. 宣言的コンビネータ (Declarative Combinators)
| コンビネータ | 構文 / 役割 | 説明 |
|---|---|---|
| `@zip_with($v1, $v2, fn)` | `(View, View, 関数名) -> Loop` | 2つのビューの対応要素に関数 `fn` を適用 |
| `@sum($view)` または `\|> @sum()` | `(View) -> i64` | ビューの全要素のスカラ合計をレジスタアキュムレータで集計 |
| `@reduce($view, $init, fn)` | `(View, 初期値, 関数名) -> i64` | アキュムレータ関数 `fn` を用いて単一のスカラ値へ畳み込み |

#### 3. AI-Native 3×3 行列乗算 実装例
```pulse
...
// 各 (i, j) 要素は、「A の i 行目」と「B の j 列目」の内積
for $i in 0..3 {
    let $row_i = @row($a, $i, 3);
    ...
    for $j in 0..3 {
        ...
        // 内積 (Dot Product): 行と列を zip して mul を適用し、合計
...
// 出力 (30, 24, 18, 84, 69, 54, 138, 114, 90)
for $k in 0..9 {
    @println($c[$k]);
}
```
