// lang.rs - PulseLang: Time-Native Stream DSL, Single-Pass Compiler & Real-Time VM
//
...
pub enum TokenKind {
    ...
    Ident,
    Number(i64),
    TimeLiteral(u64), // In nanoseconds
    StringLit,
            ...
            let kind = match b {
                ...
                b'&' => {
                    if self.peek_next() == Some(b'&') {
                        ...
                        return Err("Single '&' not supported, use '&&'");
                    }
                }
                ...
                b'0'..=b'9' => {
                    ...
                    // Check for time suffixes: ns, us, ms, s
    ...
    fn skip_whitespace_and_comments(&mut self) {
        while self.pos < self.src.len() {
            ...
            if b == b'\n' {
                ...
                self.pos += 1;
            } else if b == b'/' && self.peek_next() == Some(b'/') {
                // Line comment
                while self.pos < self.src.len() && self.src[self.pos] != b'\n' {
                    self.pos += 1;
                }
...
pub const NATIVE_PRINTLN: u8 = 1;
pub const NATIVE_SYS_TSC: u8 = 2;
pub const NATIVE_NET_RTT: u8 = 3;
pub const NATIVE_NET_SET_RATE: u8 = 4;
pub const NATIVE_GPU_CAPTURE: u8 = 5;
pub const NATIVE_NET_SEND: u8 = 6;
...
impl<'a> Compiler<'a> {
    pub fn new(src: &'a [u8], tokens: &'a [Token]) -> Self {
        Self {
            ...
            str_pool_len: 0,
            vars: [VarSymbol { name_start: 0, name_len: 0, idx: 0 }; MAX_VARS],
            var_count: 0,
        }
    }
    ...
    fn resolve_var(&mut self, token: Token) -> Result<u8, &'static str> {
        let name = &self.src[token.start..token.start + token.len];
        for i in 0..self.var_count {
            let v = &self.vars[i];
            if &self.src[v.name_start..v.name_start + v.name_len] == name {
                return Ok(v.idx);
            }
        }
        if self.var_count >= MAX_VARS {
            return Err("Max variables reached (32)");
        }
        let idx = self.var_count as u8;
        self.vars[self.var_count] = VarSymbol {
    ...
    // Function: compile
    // Description: Compile token stream into bytecode without heap allocations.
    // Worst-case execution time: ~45_000 ns
        ...
        match self.peek().kind {
            TokenKind::Let => {
                ...
                if !self.match_token(TokenKind::Eq) {
                    return Err("Expected '=' in variable declaration");
                }
                self.expression()?;
            ...
            TokenKind::Within => {
                ...
                let deadline_ns = match time_tok.kind {
                    ...
                    _ => return Err("Expected time literal after 'within' (e.g. 500us)"),
                };
                ...
                if !self.match_token(TokenKind::LBrace) {
                    return Err("Expected '{' after within clause");
                }
                while self.peek().kind != TokenKind::RBrace && self.peek().kind != TokenKind::Eof {
                    self.statement()?;
                }
            ...
            TokenKind::Pipeline => {
                ...
                if !self.match_token(TokenKind::LBrace) {
                    return Err("Expected '{' after pipeline name");
                }
                while self.peek().kind != TokenKind::RBrace && self.peek().kind != TokenKind::Eof {
                    self.statement()?;
                }
            ...
            TokenKind::On => {
                self.advance();
                let _evt = self.advance();
                if self.match_token(TokenKind::LParen) {
                    let _param = self.advance();
                    self.match_token(TokenKind::RParen);
                }
                if !self.match_token(TokenKind::LBrace) {
                    return Err("Expected '{' after on block");
                }
                while self.peek().kind != TokenKind::RBrace && self.peek().kind != TokenKind::Eof {
                    self.statement()?;
                }
            ...
            TokenKind::Budget => {
                ...
                let _time = self.advance();
                self.match_token(TokenKind::Semi);
            }

            TokenKind::Emit | TokenKind::Return => {
                self.advance();
                self.expression()?;
                self.match_token(TokenKind::Semi);
            }
            ...
            _ => {
                // Expression statement or variable assignment
                let next = self.peek();
                if next.kind == TokenKind::Ident && self.tokens.get(self.current + 1).map(|t| t.kind) == Some(TokenKind::Eq) {
                    let ident = self.advance();
            ...
            TokenKind::Ident => {
                ...
                if self.peek().kind == TokenKind::LParen {
                    ...
                    let func_id = match name {
                        b"print" => NATIVE_PRINT,
                        b"println" => NATIVE_PRINTLN,
                        b"sys.tsc" => NATIVE_SYS_TSC,
                        b"net.rtt" => NATIVE_NET_RTT,
                        b"net.set_rate" => NATIVE_NET_SET_RATE,
                        b"gpu.capture" => NATIVE_GPU_CAPTURE,
                        b"net.send" => NATIVE_NET_SEND,
...
impl<'a> VM<'a> {
    ...
    // Function: run
    // Description: Execute compiled bytecode with maximum step limit for guaranteed WCET.
    // Worst-case execution time: ~30_000 ns (for 1000 instructions)
    pub fn run(&mut self, tsc_freq_hz: u64) -> Result<(), &'static str> {
        ...
        while self.ip < self.code.len() && steps < MAX_VM_STEPS {
            ...
            match op {
                ...
                OP_PUSH_CONST => {
                    ...
                    self.ip += 8;
                    self.push(i64::from_be_bytes(bytes))?;
                }
                ...
                OP_DROP => {
                    serial_println!("[PULSE_VM] Deadline exceeded: FRAME DROPPED");
                }
        ...
        if steps >= MAX_VM_STEPS {
            return Err("VM execution cycle budget exceeded (WCET guard triggered)");
        }
...
// Description: Convenience function to tokenize, compile, and execute a PulseLang script.
// Worst-case execution time: ~90_000 ns
pub fn run_pulse_script(src: &[u8], tsc_freq_hz: u64) -> Result<(), &'static str> {
    let mut tokens = [Token::empty(); MAX_TOKENS];
    let mut lexer = Lexer::new(src);
    let _token_count = lexer.tokenize(&mut tokens)?;
